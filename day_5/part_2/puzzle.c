...
int get_solution_from_array(int *original_array, int array_size) {
  ...
  for (index = 0; index < array_size; index += 4) {
    ...
    int mode_third_parameter = get_n_position(instruction, 4);
    if (operation == 99) {
      ...
      int third_parameter =
          mode_third_parameter == 0 ? original_array[index + 3] : index + 3;
      if (operation == 1) {
        ...
        original_array[third_parameter] =
